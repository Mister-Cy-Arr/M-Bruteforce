...
if platform.python_version().split('.')[0] != '2':
	cetak('!m[!] Loe menggunakan python versi %s boejank , coba loe pakai versi 2.x.x'%v().split(' ')[0])
	sys.exit()
import cookielib,re,urllib2,urllib,threading
try:
  import mechanize
except ImportError:
	cetak('!m[!] KayaknyaNya Module !cmechanize!m belum loe di install boejank...\n!h[!] pip2 install mechanize')
	sys.exit()
br = 0
log = 0
id_bteman = []
...
def tampilhasil(akun,sandi,data):
	...
	cetak('!m[*] Gagal boejank    !c'+str(salah))
	i = inputD('[?] Kalau loe ngak Puas dengan Hasil, loe coba aja lagi boejank (lanjut/capek)',['L','C'])
	if i.upper() == 'L':
		return crack(data)
	else:
		return menu()
def crack0(data,sandi):
	akun = []
	cetak('!h[*] tunggu boejank lagi MengCrack !k%d Akun !hdengan sandi !m[!k%s!m]'%(len(data),sandi))
...
def simpan():
	if len(id_bgroup) != 0:
		cetak('!h[*] Lagi Menyimpan hasil dari Group Si boejank (loe)')
		try:
			open(os.sys.path[0]+'/M-BFgroup.txt','w').write('\n'.join(id_bgroup))
			cetak('!h[!] Berhasil meyimpan !cM-BFgroup.txt')
		except:
			cetak('!m[!] Gagal meyimpannya boejank, maaf y 😁')
	if len(id_bteman) != 0:
		cetak('!h[*] Menyimpan hasil daftar Teman...')
		try:
			open(os.sys.path[0]+'/M-BFteman.txt','w').write('\n'.join(id_bteman))
			cetak('!h[!] Berhasil meyimpan !cM-BFteman.txt')
		except:
			cetak('!m[!] Gagal meyimpannya boejank, maaf y 😁')
def keluar():
	simpan()
	cetak('!m[!] Kalau Capek Keluar aja Boejank')
	sys.exit()
def inputD(x,v=0):
	while 1:
		try:
		...
		if v:
			if a.upper() in v:
				...
				cetak('!m[!] Masukan Opsinya Boejank...')
				continue
		else:
			if len(a) == 0:
				cetak('!m[!] Masukan dengan benar boejank')
				continue
			else:
				break
	return a
def inputM(x,d):
	while 1:
		try:
			...
			cetak('!m[!] Pilihan tidak ada boejank')
			continue
		if i in d:
			break
		else:
...
def buka(d):
	try:
		...
		cetak('\r!m[!] Gagal membuka boejank !p'+str(d))
		keluar()
